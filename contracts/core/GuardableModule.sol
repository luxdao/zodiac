 ...
 * @author Gnosis Guild (Zodiac); modified by Lux Industries Inc
 * @notice Base contract for modules that can execute transactions through a Safe (avatar)
 * @dev Zodiac's GuardableModule as deployed behind the Lux DAO module masters. It differs
 * from zodiac-core 4.x on purpose: setAvatar, setTarget and setGuard are gated on the
 * avatar rather than an owner, setGuard performs no ERC-165 check, guards are called
 * through IGuard.checkTransaction rather than IModuleGuard.checkModuleTransaction, and
 * there is no execAndReturnData. The live masters depend on this shape; do not
 * reconcile it with upstream without a migration plan for them.
 *
